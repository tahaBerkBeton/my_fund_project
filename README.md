...
The project uses **SQLAlchemy** with an **SQLite** database to persist data. The structure is designed to keep track of all aspects of fund management. Here’s a breakdown of the chosen database structure:

- **`funds` Table**  
  - **Purpose:** Stores core information for each fund.  
  - **Key Fields:**  
    - `id`: Primary key.
    - `name`: Unique name for the fund.
    - `creation_date`: Timestamp when the fund was created.
    - `current_cash`: Available cash in the fund.
    - `last_update`: Timestamp of the last update operation.
  
- **`fund_positions` Table**  
  - **Purpose:** Records the portfolio composition for each fund (i.e., which stocks are held).  
  - **Key Fields:**  
    - `id`: Primary key.
    - `fund_id`: Foreign key linking to the `funds` table.
    - `ticker`: Stock symbol (e.g., AAPL, TSLA).
    - `shares_held`: Number of shares held.
    - `last_purchase_price`: Price per share at the last purchase.
    - `last_purchase_date`: Date of the last purchase.
  
- **`fund_valuations` Table**  
  - **Purpose:** Captures snapshots of a fund’s total value each time it is updated, enabling historical tracking of performance.  
  - **Key Fields:**  
    - `id`: Primary key.
    - `fund_id`: Foreign key linking to the `funds` table.
    - `valuation_date`: Timestamp when the valuation was recorded.
    - `total_value`: Calculated total value of the fund at the time of the snapshot (cash + market value of stocks).
  
- **`operations` Table**  
  - **Purpose:** Logs every operation (e.g., fund creation, buying, selling) for audit and historical purposes.  
  - **Key Fields:**  
    - `id`: Primary key.
    - `fund_id`: Foreign key linking to the `funds` table.
    - `ticker`: Stock symbol involved in the operation (if applicable).
    - `operation_type`: Type of operation (e.g., CREATE, BUY, SELL).
    - `shares`: Number of shares involved in the transaction.
    - `price`: Price per share during the operation.
    - `operation_date`: Timestamp when the operation occurred.

This database design ensures that all fund activities are recorded, providing a complete historical view that can be used for performance tracking, auditing, and further analysis.
